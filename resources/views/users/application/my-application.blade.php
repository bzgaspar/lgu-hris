                                ...
                                <td>
                                    <div class="btn-group" role="group" aria-label="Basic example">
                                        <a href="/users/application/{{ $app->publication->id }}/edit/{{ $app->id }}"
                                            class="btn btn-outline-warning fw-bold btn-sm text-white">View</a>
                                        @if ($app->status == 2)
                                        {{-- @elseif ($app->trashed())
                                            <form action="{{ route('users.application.resubmit', $app->id) }}"
                                                ...
                                                <button type="submit" onclick="return confirm('Are you sure?')"
                                                    class="btn btn-outline-success fw-bold btn-sm text-white">Resubmit</button>
                                            </form> --}}
                                            {{-- <form action="{{ route('users.application.delete', $app->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="btn btn-outline-danger fw-bold btn-sm rounded-0 rounded-end">Delete</button>
                                            </form> --}}
                                        @elseif($app->status == 1)
                                            <form action="{{ route('users.application.destroy', $app->id) }}"
                                                ...
                                                @method('DELETE')
                                                <button type="submit" onclick="return confirm('Are you sure?')"
                                                    class="btn btn-outline-danger fw-bold btn-sm rounded-0 rounded-end">Cancel</button>
                                            </form>
                                        @endif
                                    </div>
                                </td>
